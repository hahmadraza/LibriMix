...
def create_speakers_dataframe(librispeech_dir):
    ...
    speakers_metadata_path = os.path.join(librispeech_dir, 'SPEAKERS.csv')
    ...
    speakers_metadata = pd.read_csv(speakers_metadata_path)
...
def check_already_generated(md_dir, librispeech_dir):
    ...
    original_librispeech_dirs = ['test-clean','train-clean-360']
